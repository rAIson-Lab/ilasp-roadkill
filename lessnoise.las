...
trait(bradypus_variegatus,litters_or_clutches_per_y_cat,low).
...
trait(mus_musculus,diet_invertebrates_cat,high).
trait(mus_musculus,diet_scavenger_cat,low).
trait(mus_musculus,population_density_indkm2_cat,high).
...
trait(nectomys_squamipes,diet_invertebrates_cat,high).
trait(nectomys_squamipes,diet_scavenger_cat,low).
...
trait(oligoryzomys_flavescens,diet_invertebrates_cat,high).
trait(oligoryzomys_flavescens,diet_scavenger_cat,low).
...
trait(orthogeomys_hispidus,diet_invertebrates_cat,low).
trait(orthogeomys_hispidus,diet_scavenger_cat,low).
...
trait(rattus_norvegicus,diet_invertebrates_cat,high).
trait(rattus_norvegicus,diet_scavenger_cat,low).
...
trait(rattus_rattus,diet_invertebrates_cat,high).
trait(rattus_rattus,diet_scavenger_cat,low).
trait(rattus_rattus,population_density_indkm2_cat,high).
...
trait(saimiri_sciureus,litters_or_clutches_per_y_cat,low).
...
#pos({roadkill_risk_high(bradypus_variegatus)}, {}).
#pos({roadkill_risk_high(cabassous_tatouay)}, {}).
#pos({roadkill_risk_high(cabassous_unicinctus)}, {}).
#pos({roadkill_risk_high(caluromys_derbianus)}, {}).
#pos({roadkill_risk_high(caluromys_lanatus)}, {}).
...
#pos({roadkill_risk_high(sciurus_variegatoides)}, {}).
...
#neg({roadkill_risk_high(didelphis_albiventris)}, {}).
...
#neg({roadkill_risk_high(leopardus_geoffroyi)}, {}).
...
#neg({roadkill_risk_high(philander_opossum)}, {}).
